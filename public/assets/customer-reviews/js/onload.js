...
$('.IndexPageContainer').css('height',windowHeight);
...
var Book_v5 = {
    ...
    init: function() {
		...
		$('#fb5-book').turn({
			...
			when: {
				...
				turned: function(e, page) {
					$('.twitterLoader.HidethisLoader').show();
					if(windowWidth < 1024){
						if(windowWidth > windowHeight){
							turnwrapper = parseInt(turnwrapper) + 20;
						}
					}
					console.log('turned--'+page);
					$('.BodyCOntent').css('max-height',turnwrapper);
					//$('.turn-page-wrapper').removeClass('getZindex');
					//$('#PageNo_'+page).parents('.turn-page-wrapper').addClass('getZindex');
					playPauseAudio(page);
					if (page > 1) {
		...
		$('#fb5-book').bind('turning', function(e, page) {
			console.log('turning ---'+page);
			turnwrapper = $('.turn-page-wrapper').height();
			turnwrapper = parseInt(turnwrapper)-105;
			Opnpage(page);
		});
		Book_v5.arrows();
	...
	book_position: function() {
		...
		//alert('book_height'+book_height);

		var book_width	= this.widthBook();
		//alert('book_width'+book_width);

		var half_height	= (  (book_height/2)+this.toolsHeight/2   );
		//alert('half_height'+half_height);

		var half_width	= (  book_width/2 );
        //alert('half_width'+half_width);
...
var stopWrtitter = 1;
var PlayAudio = 0;
function typeWriter(txt, n, eid) {
	clearTimeout(typeWriteTimeout);
	if(undefined !== txt && n < (txt.length) && $('#'+eid).attr('data-pause') == 0){
		$('#'+eid).html(txt.substring(0, n+1));
		n++;
		$('#'+eid).attr('data-length',n);
		typeWriteTimeout = setTimeout(function(){
		  typeWriter(txt, n, eid);
		},80);
	}else{
		if($('#'+eid).attr('data-pause') == 0){
			$('#'+eid).attr('data-complete','1');
			var npage = eid.split('_');
			npage = npage[1];
			$('.RplyTextLAbel_'+npage).show();
			$('#ReviewRplyTextHide_'+npage).show();

		}
	}
}

function playPauseAudio(page){

	$('.twitterLoader.HidethisLoader').hide();
	$.each(ALLAUDIOS,function(aid,aele){
		aele.pause();
		if($('#ReviewTextShow_'+npage).attr('data-pause') == 0){
			aele.currentTime = 0;
		}
		var npage = aid.split('_');
		npage = npage[1];

		if($('#ReviewTextShow_'+npage).attr('data-complete') == 0 && $('#ReviewTextShow_'+npage).attr('data-pause') == 0){
			$('#ReviewTextShow_'+npage).html("");
		}
	});
	if (page > 1) {
		if($("#ReviewTextShow_"+CURRENTPAGENUM).attr('data-Autopause') == 1){
			$("#ReviewTextShow_"+CURRENTPAGENUM).attr('data-pause','0');
			$("#ReviewTextShow_"+CURRENTPAGENUM).attr('data-Autopause','0');
		}
		var cpage = parseInt(page);
		if(ALLAUDIOS['Audio_'+page] == '' || ALLAUDIOS['Audio_'+page] == undefined || ALLAUDIOS['Audio_'+page] == null){
			cpage = parseInt(page) - 1;
		}
		CURRENTPAGENUM = cpage;
		CURRENTPAGETYPE = 0;
		if($('#ReviewTextShow_'+cpage).attr('data-audio') == 1){
			if($('#ReviewTextShow_'+cpage).attr('data-pause') == 0){
				ALLAUDIOS['Audio_'+cpage].play();
			}
		}else{
			var revStart = $('#ReviewTextShow_'+CURRENTPAGENUM).attr('data-length');
			if($('#ReviewTextShow_'+CURRENTPAGENUM).attr('data-complete') == 0){
				CURRENTPAGETYPE = 1;
				var revTxt = $('#ReviewTextHide_'+CURRENTPAGENUM).text();
				revStart = parseInt(revStart);
				typeWriter(revTxt, revStart, 'ReviewTextShow_'+CURRENTPAGENUM);
			}
		}
	}else{
		$("#ReviewTextShow_"+CURRENTPAGENUM).attr('data-pause','1');
		$("#ReviewTextShow_"+CURRENTPAGENUM).attr('data-Autopause','1');
	}
...
function showMeTheTime(){
	if(ALLAUDIOS['Audio_'+CURRENTPAGENUM] != '' && ALLAUDIOS['Audio_'+CURRENTPAGENUM] != undefined && ALLAUDIOS['Audio_'+CURRENTPAGENUM] != null) {
		if(ALLAUDIOS['Audio_'+CURRENTPAGENUM].currentTime > 0 && CURRENTPAGETYPE == 0){
			var revStart = $('#ReviewTextShow_'+CURRENTPAGENUM).attr('data-length');
			if($('#ReviewTextShow_'+CURRENTPAGENUM).attr('data-complete') == 0){
				CURRENTPAGETYPE = 1;
				var revTxt = $('#ReviewTextHide_'+CURRENTPAGENUM).text();
...
function MuteAudio(PNO){

	ALLAUDIOS['Audio_'+PNO].muted = true;
	$('.my_image_'+PNO).attr('src','assets/customer-reviews/img/mute.png');
	$('.MuteAUDIO_'+PNO).attr('onclick',"UnmuteAudio('"+PNO+"')");
}

function UnmuteAudio(PNO){
	$('.my_image_'+PNO).attr('src','assets/customer-reviews/img/speaker.png');
	$('.MuteAUDIO_'+PNO).attr('onclick',"MuteAudio('"+PNO+"')");
	ALLAUDIOS['Audio_'+PNO].muted = false;
}

function PauseAudio(PNO){
	ALLAUDIOS['Audio_'+PNO].pause();
	$("#ReviewTextShow_"+PNO).attr('data-pause','1');
	$('.Pause_'+PNO).attr('src','assets/customer-reviews/img/play.png');
	$('.PauseAudio_'+PNO).attr('onclick',"PlayPausedAudio('"+PNO+"')");
}
function PlayPausedAudio(PNO){
	CURRENTPAGETYPE = 0;
	$('.Pause_'+PNO).attr('src','assets/customer-reviews/img/pause.png');
	$('.PauseAudio_'+PNO).attr('onclick',"PauseAudio('"+PNO+"')");
	ALLAUDIOS['Audio_'+PNO].play();
	$("#ReviewTextShow_"+PNO).attr('data-pause','0');
}


var FolderNumber = '';
function OpnPrepage(page,folderNumber){
	$('.IndexLi').removeClass('CurrentACtive');
	var active_page = page / 2;
	$('.Index_'+active_page).addClass('CurrentACtive');
	var book = $('#fb5-book');
	if(page <= TotalPages){
		FolderNumber = folderNumber;
		book.turn("page",page);
	}
	if($('.Humberg').is(':visible')){
		$('.IndexPageContainer').hide();
	}
}

function OPEN_FROM_PAGINATION(page){
    console.log(page,"page")
	$('.Index_'+page).click();
}

function Opnpage(page) {
	$('.paginationSPAN').removeClass('active');
	$('.PaginationPage_'+page).addClass('active');
	var book = $('#fb5-book');
	if (!book.turn('hasPage', page)) {
		console.log('Opnpage Created');
		var element = $('<div />').html('<div class="twitterLoader"><span class="loading_spinner"></span></div>');
		var element2 = $('<div />').html('<div class="twitterLoader"><span class="loading_spinner"></span></div>');
		var adpage = page;
		if (page % 2 === 0) {
			var adpage = page;
			console.log(adpage);
			...
			if(page < TotalPages){
				book.turn('addPage', element2, parseInt(page)+1);
			}
		}else {
			var adpage = parseInt(page) - 1;
			book.turn('addPage', element, page);
			if(page < TotalPages){
				book.turn('addPage', element2, parseInt(page)-1);
			}
		...
		$.ajax({
			...
			data:{
                "_token": "{{ csrf_token() }}",
                'page':adpage,'FolderNumber':FolderNumber},
			success:function(response){
				FolderNumber = '';
				if(page < TotalPages){
					element.html(response.HTML1);
					var element3 = element2;
				}else{
					var element3 = element;
				}
				playPauseAudio(adpage);
				var newHTML = '<div class="fb5-cont-page-book ImageOnMobile"></div>';
				element3.attr('data-background-image',response.HTML2);
				element3.addClass('BackImage');
				element3.addClass('ImageNotONPHONE');
				element3.html(newHTML);


				$('#fb5-book .turn-page').each(function(index, element){
					var page_div=$(element);
					var data_background_image=page_div.attr('data-background-image');
					if(data_background_image!="" &&  data_background_image!=undefined){
						page_div.css('background-image', 'url("'+data_background_image+'")');
				...
				$("#fb5 .fb5-cont-page-book").css('height',HEIGHT_BOOK+'px');
				$("#fb5 .fb5-cont-page-book").css({'transform-origin':'0 0','-ms-transform-origin':'0 0','-webkit-transform-origin':'0 0'});
				$('.ImageNotONPHONE').parents('.turn-page-wrapper').addClass('hidePageOnPhone');
...
function showPage(pages){
	var usedpages = $('#UsedPages').val();
	var currPage = page;
	if(page > OLDPAGE && page > MAXPAGE){
		MAXPAGE = page;
		page = parseInt(usedpages) + 2;
		var book = $(this);

		if (!book.turn('hasPage', page) && usedpages < TotalPages) {
			var element = $('<div />').html('');
			var element2 = $('<div />').html('');

			if(RTLIMAGE == 0){
				book.turn('addPage', element, page);
				if(page < TotalPages){
					book.turn('addPage', element2, page+1);
				}
				RTLIMAGE = 1;
				console.log('page');
				console.log(page);
			} else {
				book.turn('addPage', element2, page);
				if(page < TotalPages){
					book.turn('addPage', element, page+1);
				}
				RTLIMAGE = 0;
				console.log('page');
				console.log(page);
			...
			setTimeout(function(){
				$.ajax({
					...
					success:function(response){
						...
						usedpages = parseInt(usedpages)+2;
						$('#UsedPages').val(usedpages);
						console.log(page);
						...
						setTimeout(function(){
							var newHTML = '<div class="fb5-cont-page-book"></div>';
							element3.attr('data-background-image',response.HTML2);
							element3.addClass('ImageNotONPHONE');
							element3.addClass('BackImage');
							element3.html(newHTML);
							setTimeout(function(){
								//console.log($('#fb5-book .turn-page-wrapper .BackImage').length);
								$('#fb5-book .BackImage').each(function(index, element){
									var page_div=$(element);
									//console.log(page_div);
									var data_background_image=page_div.attr('data-background-image');
									//console.log(data_background_image);
									 if(data_background_image!="" &&  data_background_image!=undefined){
										 page_div.css('background-image', 'url("'+data_background_image+'")');
...
var usepages = 10;
var pageNo = '';
function ShowNextPages(Lastpage) {
	if(Lastpage > 1){
		$(".PageFirst").show();
		$(".PageFirst").attr('onclick', 'ShowPreviousPages('+Lastpage+');');
	}
	$(".pagesno").each(function(idx,obj) {
		$(obj).removeClass('hideextrapagination');
		var no = $(obj).html();
		no = parseInt(no) + 10;
		pageNo = no;
		if(pageNo > TotalReviews){
			$(obj).addClass('hideextrapagination');
		}
		$(obj).html(no);
		var TurnedPAge = 2 + (no-1)*2;
		$(obj).attr('onclick', 'OPEN_FROM_PAGINATION('+pageNo+');');
	});
	$('.pagesnoLast').attr('onclick', 'ShowNextPages('+pageNo+');');

	var TotalpageModules = TotalReviews%10;
	if(TotalpageModules > 0){
		TotalpageModules = 10 - parseInt(TotalpageModules);
		TotalpageModules = parseInt(TotalReviews)+parseInt(TotalpageModules);
	}else{
		TotalpageModules = parseInt(TotalReviews);
	}

	if(pageNo == TotalpageModules){
		$('.pagesnoLast').hide();
		$('.ENDPAGE').show();
		$('.StartPage').hide();
	} else {
...
var prepageNo = '';
function ShowPreviousPages(page){
	$(".pagesno").each(function(idx,obj) {
		$(obj).removeClass('hideextrapagination');
		var no = $(obj).html();
		no = parseInt(no) - 10;
		prepageNo = no;
		//console.log(prepageNo);
		if(prepageNo > TotalReviews){
			$(obj).addClass('hideextrapagination');
		}
		$(obj).html(no);
		var TurnedPAge = 2 + (no-1)*2;
		$(obj).attr('onclick', 'OPEN_FROM_PAGINATION('+no+');');
	});
	var ppage = page - 10;
	$('.PageFirst').attr('onclick', 'ShowPreviousPages('+ppage+');');
	$('.pagesnoLast').attr('onclick', 'ShowNextPages('+prepageNo+');');
	$('.pagesnoLast').show();
	$('.ENDPAGE').hide();
	if(ppage == 0){
		$(".PageFirst").hide();
...
function ShowIndex(){
	$('.IndexPageContainer').show();
	$('.CloseInDEX').css('display','block');
}
